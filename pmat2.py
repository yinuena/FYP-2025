...
if 'current_inputs' not in st.session_state:
    st.session_state.current_inputs = {} # New state for storing latest inputs
...
st.markdown("""
<div class="info-box">
<strong>ℹ️ Tool Guidance:</strong> Enter your pipeline's design parameters in the sidebar and click 'Generate AI Recommendation' to receive an explained material suggestion.
</div>
""", unsafe_allow_html=True)

# --- SIDEBAR (INPUT FORM AND CONTEXT) ---
with st.sidebar:
    
    st.header("Pipeline Parameters Input")
    
    # --- USER INPUT FORM (Moved to sidebar) ---
    ...
    if stats:
        ...
        # Display as simple text for space efficiency
    ...
    with st.spinner(" Generating recommendation..."):
        ...
        if result:
            ...
            st.session_state.prediction_history.append({
                ...
                'inputs': st.session_state.current_inputs, # Use the current_inputs dictionary here
                'result': result['material'],
                'confidence': result['confidence']
            })
            st.rerun() 

# --- DISPLAY RESULTS (Main Area Focus) ---
if st.session_state.prediction_made and st.session_state.prediction_result:
    result = st.session_state.prediction_result
    inputs = st.session_state.current_inputs
    st.markdown("---")
    st.header("AI Recommendation Results & Local XAI")
    st.success("✅ Analysis Complete! Review the recommendation and its engineering justification below.")
    ...
    # --- P-T Plot Pinpoint (Visual Trust Factor) ---
    ...
    <strong>Insight:</strong> The **star** marker shows your input parameters in context. This visual check increases trust: if the star is within a material cluster, the prediction is highly validated by historical data.
    </div>
    """, unsafe_allow_html=True)
    ...
    with col_xai_1:
        # Alternatives and Reasoning (Local XAI)
        if result['alternatives']:
            st.subheader("Alternatives & Rejection Analysis (Local XAI)")
            st.markdown("""
            <div class="warning-box">
            <strong>🔍 Rejection Analysis:</strong><br>
            These alternative materials were ruled out primarily because of engineering constraints or non-optimal fit with your input parameters.
            </div>
            """, unsafe_allow_html=True)
...
# --- DATASET VISUALIZATIONS & GLOBAL XAI CONTEXT (Now minimized) ---
if df is not None:
    st.markdown("---")
    with st.expander("📊 **Expand for Global XAI & Historical Data Analysis**"):
        st.header("Global Dataset Analysis & Context")
        st.markdown("These visualizations provide the **Global Context**, showing the historical relationship between design parameters and material selection.")
        
        # 2. Product vs. Service Profile (Categorical Breakdown)
        st.subheader("1. Corrosivity Profile: Product and Service Breakdown")
        ...
        st.subheader("2. Design Pressure Distribution by Material Type")
...
if st.session_state.prediction_history:
    ...
    history_display = []
    # Use st.session_state.current_inputs to populate if needed, but history is more reliable
